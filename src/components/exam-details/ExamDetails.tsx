...
import MakeAIForm from './MakeAIForm';
import Notification from '../../ui/Notification';
import { fontSize } from '../../lib/utils';
import localStorageCache from '../../lib/localStorage';
import ExamInfoModal from './exam/ExamInfoModal';
import DeleteCategoryModal from './exam/DeleteCategoryModal';
import TemplateModal from './exam/TemplateModal';
import EditCategoryModal from './exam/EditCategoryModal';
...
export default function ExamDetails() {
  ...
  // Fix invalid escape sequences inside JSON string values (character by character parser)
  const handleDeleteCategory = async () => {
    if (!examType || deleteConfirmation !== examType.name) return;

    setIsDeleting(true);
    try {
      // 1. Delete all results for exams in this category
      const { data: catExams, error: examsError } = await supabase
        .from('exams')
        .select('id')
        .eq('categoryId', id);
      ...
      if (catExams && catExams.length > 0) {
        const examIds = catExams.map(e => e.id);
      ...
      // Update cache to remove deleted category
      const cachedExamTypes = localStorageCache.get<any[]>(localStorageCache.keys.EXAM_CATEGORIES) || [];
      const updatedCache = cachedExamTypes.filter(exam => exam.id !== id);
  ...
  // Start timer when delete modal opens
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-gray-100 font-sans antialiased select-none relative">
      ...
      <DeleteCategoryModal
        ...
        onConfirmationChange={(v) => setDeleteConfirmation(v)}
      />
