...
export default function MakeScanQuestions({ show, onClose, userProfile, categoryId, availableSubjects }: MakeScanQuestionsProps) {
  ...
  const isScheduleValid = accessType === 'anytime' || (startDate !== '' && endDate !== '' && startDateTime !== 'anytime' && endDateTime !== 'anytime' && !isNaN(new Date(startDateTime).getTime()) && !isNaN(new Date(endDateTime).getTime()) && new Date(startDateTime) < new Date(endDateTime));
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 mt-1.5">
                            <input
                              ...
                              className="flex-1 bg-zinc-100 dark:bg-gray-900 border border-zinc-300 dark:border-gray-800 rounded-xl px-3 py-2.5 text-zinc-900 dark:text-white focus:ring-1 focus:ring-blue-500 focus:outline-none transition-all light:[color-scheme:light] dark:[color-scheme:dark] text-sm" />
                            <select
                              value={startHour}
                              onChange={(e) => {
            ...
            {step === 'details' ? (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={!examName || !isScheduleValid}
