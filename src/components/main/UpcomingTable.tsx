...
export default function UpcomingTable({ exams, loading, hasMore, onLoadMore }: UpcomingTableProps) {
  ...
  if (exams.length === 0 && !loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center opacity-60">
        <p className="text-gray-500 dark:text-gray-400 font-medium text-sm">No upcoming exams found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-gray-900/30 rounded-xl border border-black/8 dark:border-white/10 shadow-xs overflow-hidden transition-colors">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead
              className="bg-zinc-50 dark:bg-gray-900/60 text-zinc-500 dark:text-gray-400 font-semibold border-b border-black/8 dark:border-white/10 text-xs">
              <tr>
              ...
              {exams.map((exam) => (
                ...
                >
                  ...
                  <td className="px-4 sm:px-5 py-3 sm:py-4">
                    <span
                      className={`px-2 py-0.5 rounded-full font-medium ${exam.status === 'Completed' || exam.status === 'Ongoing'
                  ...
                  <td className="px-4 sm:px-5 py-3 sm:py-4 text-right">
                    <span
                      className={`px-2 py-0.5 rounded-full font-semibold capitalize ${exam.difficulty === 'hard' || exam.difficulty === 'expert'
      ...
      {hasMore && (
        <div className="flex justify-center mt-6">
          <button
            onClick={onLoadMore}
            disabled={loading}
            className="px-4 sm:px-5 py-2 bg-white dark:bg-gray-900 hover:bg-zinc-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed border border-zinc-200 dark:border-gray-700 rounded-xl font-medium transition-all flex items-center gap-2 cursor-pointer text-zinc-700 dark:text-gray-300 text-xs">
            {loading ? (
