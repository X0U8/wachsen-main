    ...
    <div className="max-w-4xl w-full mx-auto space-y-4">
      {revisionList.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800/80 rounded-2xl shadow-sm">
          <p className="text-zinc-500 dark:text-zinc-400 font-medium" style={{ fontSize: fontSize.xs }}>No revision logs created yet.</p>
          <p className="text-zinc-450 dark:text-zinc-550 mt-1" style={{ fontSize: fontSize.xs }}>Complete an exam and click "Create Revision Log" on the results page to generate one.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex-1 min-w-0 space-y-1">
                  <h3 className="font-bold text-zinc-900 dark:text-white truncate" style={{ fontSize: fontSize.sm }}>
                    {examName}
                  </h3>
                  <div className="text-zinc-500 dark:text-zinc-400 font-medium" style={{ fontSize: fontSize.xs }}>
                    {dateString}
                  </div>
                </div>
                {log.question_count !== undefined && (
                  <span className="font-semibold whitespace-nowrap flex-shrink-0 bg-zinc-100 dark:bg-zinc-800 text-zinc-650 dark:text-zinc-405 px-2.5 py-1 rounded-lg" style={{ fontSize: fontSize.xs }}>
