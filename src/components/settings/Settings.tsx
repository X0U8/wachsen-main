...
export default function Settings() {
  ...
  const Toggle = ({ on, onToggle }: { on: boolean; onToggle: () => void }) => (
    <button onClick={onToggle}
      className={`relative w-10 h-5 rounded-full transition-colors cursor-pointer shrink-0 ${on ? 'bg-[#007AFF]' : 'bg-zinc-300 dark:bg-gray-700'}`}>
      <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${on ? 'translate-x-5' : 'translate-x-0'}`} />
    </button>
  );
  ...
  return (
    <>
      <div className="flex flex-col min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-gray-100 font-sans antialiased">
        <header className="sticky top-0 z-40 w-full px-4 sm:px-6 py-3 bg-white/80 dark:bg-black/80 backdrop-blur-md border-b border-zinc-200 dark:border-gray-900/80 flex items-center gap-3 transition-colors duration-300">
        ...
        <main className="flex-1 max-w-2xl w-full mx-auto p-4 sm:p-6 space-y-4">
          ...
          <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-gray-800 rounded-2xl p-4 sm:p-5 space-y-4">
            ...
            <div className="pt-2 border-t border-zinc-100 dark:border-gray-800">
              <Row label="Use my own key" sub="Bypass credit deduction" right={<Toggle on={useOwnKey} onToggle={toggleUseOwnKey} />} />
            </div>
          </div>

          <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-gray-800 rounded-2xl p-4 sm:p-5 space-y-4">
            <Row label="Theme" sub={theme === 'dark' ? 'Dark mode' : 'Light mode'}
              right={
                <button onClick={toggleTheme} className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${theme === 'dark' ? 'bg-blue-600' : 'bg-zinc-300'}`}>
